...
function startDashboard() {
  ...
  importFiles(file1, file2).then(function (results) {
    ...
    // Call function to create mirrored beeswarm
    createMirroredBeeswarm();
  });
}
...
// Function to create the mirrored beeswarm. incomeperson (x),  alcconsumption (radius), and lifeexpectancy (fill) variables
function createMirroredBeeswarm() {
  // Filter the data to remove entries with missing incomeperperson or alcconsumption or lifeexpectancy values
  currentData = globalDataCapita.filter(function (d) {
    return d.incomeperperson != "" && d.alcconsumption != "" && d.lifeexpectancy != "";
  });
  ...
  const svg = d3.select("#mirroredBeeswarm")
  .append("svg")
  .attr("width", beeWidth)
  ...
  // Create x, radius and fill scales for the plot
  const xScale = d3
    .scaleLog()
    .domain([
      d3.min(currentData, (d) => d.incomeperperson),
  ...
  // Create tick marks and labels for the x axis
  var xTicks = [];
  for (let index = 0; index <= 1; index += 0.15) {
    xTicks.push(Math.round(xScale.invert(index * (beeWidth))));
  }
  xTicks.shift();
  ...
  svg.append("g")
      .attr("class", "x-axis")
      .attr("transform", `translate(0, ${beeHeight - margin.bottom})`)
  ...
  svg.append('g')
    .selectAll("circle")
    .data(dodge(currentData, {radius: d => radiusScale(d.alcconsumption), x: d => xScale(d.incomeperperson)}))
    .enter()
    .append("circle")
    .attr("class", "bee data")
      .attr("cx", d => d.x)
      .attr("cy", d => beeHeight / 2 - radiusScale(d.data.alcconsumption) - padding - d.y)
      .attr("r", d => radiusScale(d.data.alcconsumption))
      .attr("fill", d => d3.interpolateBlues(fillScale(d.data.lifeexpectancy)))
    .on("mouseover", handleMouseOver) // Function to handle mouseover event
    .on("mouseout", handleMouseOut)   // Function to handle mouseout event
    .append("title")
   ...
   svg.append("text")
    .attr("class", "x-axis-label")
    .attr("x", beeWidth / 2)
    .attr("y", beeHeight + margin.top + 20)
    .style("text-anchor", "middle")
    .text("Income per person");

}

// Dodge helper function to not plot the circles overlapping
dodge = (data, {radius, x}) => {
  const padding = 5;
  const circles = data.map(d => ({x: x(d), radius: (radius(d)*2 + padding)**2, data: d})).sort((a, b) => a.x - b.x);
  const epsilon = 1e-3;
  ...
  function intersects(x, y) {
    let a = head;
    while (a) {
      if (a.radius - epsilon > (a.x - x) ** 2 + (a.y - y) ** 2) {
        return true;
      }
      a = a.next;
    }
  ...
  for (const b of circles) {
    ...
    while (head && head.x < b.x - b.radius) head = head.next;
    ...
    if (intersects(b.x, b.y = 0)) {
      ...
      b.y = Infinity;
      do {
        let y1 = a.y + Math.sqrt(a.radius - (a.x - b.x) ** 2);
        let y2 = a.y - Math.sqrt(a.radius - (a.x - b.x) ** 2);
